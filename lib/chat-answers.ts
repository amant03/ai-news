...
export function answerQuestion(question: string): string {
  ...
  if (/open.?weight|self.?host|local|run locally/i.test(q)) {
  ...
  if (/cod(e|ing)|program|software|develop/i.test(q)) {
    const c = models.filter(m => m.codingIndex != null).sort((a, b) => (b.codingIndex || 0) - (a.codingIndex || 0)).slice(0, 5);
    return c.length ? "**Top 5 Coding Models:**\n\n" + c.map((m, i) => `${i + 1}. ${fmt(m)} | Coding: ${m.codingIndex}`).join('\n\n') : "No coding-specific data available.";
  }
  ...
  if (/price|cost|pricing|how much/i.test(q)) {
  ...
  if (/compare|vs|versus|difference between/i.test(q)) {
    ...
    const names = words.filter(w => w.length > 3 && !['compare', 'vs', 'versus', 'between', 'the', 'and', 'with'].includes(w));
    const found = names.map(n => findModel(n, models)).filter(Boolean) as Model[];
    if (found.length >= 2) {
      return `**${found[0].name} vs ${found[1].name}:**\n\n| | ${found[0].name} | ${found[1].name} |\n|---|---|---|\n| Intelligence | ${found[0].intelligenceIndex || 'N/A'} | ${found[1].intelligenceIndex || 'N/A'} |\n| Speed | ${found[0].aaSpeed || 'N/A'} t/s | ${found[1].aaSpeed || 'N/A'} t/s |\n| Cost/Task | $${found[0].aaCostPerTask || 'N/A'} | $${found[1].aaCostPerTask || 'N/A'} |\n| Context | ${found[0].context || 'N/A'} | ${found[1].context || 'N/A'} |`;
    }
