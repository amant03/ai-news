...
export function toSlim(m: ModelRecord): SlimModel {
  ...
  if (Number.isFinite(m.elo)) slim.elo = m.elo;
  if (Number.isFinite(m.numVotes)) slim.numVotes = m.numVotes;
  if (Number.isFinite(m.hfDownloads)) slim.hfDownloads = m.hfDownloads;
  if (Number.isFinite(m.hfLikes)) slim.hfLikes = m.hfLikes;
  if (Number.isFinite(m.intelligenceIndex)) slim.intelligenceIndex = m.intelligenceIndex;
  if (Number.isFinite(m.codingIndex)) slim.codingIndex = m.codingIndex;
  if (Number.isFinite(m.agenticIndex)) slim.agenticIndex = m.agenticIndex;
  if (Number.isFinite(m.promptPrice)) slim.promptPrice = m.promptPrice;
  if (Number.isFinite(m.completionPrice)) slim.completionPrice = m.completionPrice;
  if (Number.isFinite(m.valueScore)) slim.valueScore = m.valueScore;
  if (Number.isFinite(m.aaSpeed)) slim.aaSpeed = m.aaSpeed;
  if (Number.isFinite(m.aaCostPerTask)) slim.aaCostPerTask = m.aaCostPerTask;
  if (Number.isFinite(m.aaVerbosity)) slim.aaVerbosity = m.aaVerbosity;
  if (Number.isFinite(m.aaLatency)) slim.aaLatency = m.aaLatency;
