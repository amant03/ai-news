...
export default function SourceLink({ href, label = 'Source', compact = false, className = '' }: SourceLinkProps) {
  ...
  return (
    <a
      ...
      className={`ring-focus inline-flex items-center gap-1.5 rounded-full border border-white/15 bg-black/45 text-white/90 backdrop-blur-md hover:border-[var(--accent)]/60 hover:bg-[var(--accent)]/20 hover:text-[var(--accent)] transition-colors ${
    ...
    <div className="flex flex-wrap gap-1.5">
      {links.map(l => (
        <a
          ...
          target="_blank"
          rel="noopener noreferrer"
          className="ring-focus inline-flex items-center gap-1 rounded-full border border-[var(--color-line)] bg-[var(--input)]/80 px-2 py-0.5 text-[10px] font-medium text-[var(--mut)] hover:text-[var(--accent)] hover:border-[var(--accent)]/40 transition-colors"
        >
