...
import data from '@/data/text-to-video-models.json';
...
export default function TextToVideoLeaderboard() {
      ...
      <main className="max-w-[1400px] mx-auto px-5 pt-8 pb-16">
        <div className="mb-8">
          ...
          <div className="flex items-center gap-3 flex-wrap">
            <h1 className="font-display text-4xl md:text-5xl font-medium tracking-tight text-[var(--fore)]">Text to Video Leaderboard</h1>
            <span className="badge-updated">Updated</span>
          </div>
          <p className="text-sm text-neutral-500 mt-2">
            Ranking of AI text-to-video generation models by Elo score from blind user comparisons in the Artificial Analysis Text to Video Arena.
          </p>
        ...
        <section className="mt-12">
          ...
          <div className="space-y-4 max-w-3xl">
            {[
              {
                q: 'Which is the best Text to Video AI model?',
                a: `${sorted[0]?.name || 'Gemini Omni Flash'} currently leads the Artificial Analysis Text to Video Arena with an Elo score of ${fmtElo(sorted[0]?.elo || 1239)}.`,
              },
              {
                q: 'What are the top Text to Video models?',
                a: `The top Text to Video models by Elo rating are: ${sorted.slice(0, 5).map((m, i) => `${i + 1}. ${m.name} (Elo ${fmtElo(m.elo)})`).join(', ')}. Rankings are based on blind user votes in the Artificial Analysis Text to Video Arena.`,
              },
              {
                q: 'How are Text to Video models ranked on this leaderboard?',
                a: 'Models are ranked using an Elo rating system derived from user votes in blind comparisons in the Artificial Analysis Text to Video Arena. Users compare two videos generated from the same prompt without knowing which model created each video. Higher Elo scores indicate a model is preferred more often by users.',
              },
            ].map((faq, i) => (
