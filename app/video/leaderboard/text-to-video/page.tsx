      ...
      <main className="max-w-[1400px] mx-auto px-5 pt-8 pb-16">
        <div className="mb-8">
          ...
          <p className="text-sm text-neutral-500 mt-2">
            Ranking of AI text-to-video generation models by Elo score from blind user comparisons in the Text to Video Arena.
          </p>
            ...
            {[
              {
                ...
                a: `${sorted[0]?.name || 'Gemini Omni Flash'} currently leads the Text to Video Arena with an Elo score of ${fmtElo(sorted[0]?.elo || 1239)}.`,
              },
              {
                q: 'What are the top Text to Video models?',
                a: `The top Text to Video models by Elo rating are: ${sorted.slice(0, 5).map((m, i) => `${i + 1}. ${m.name} (Elo ${fmtElo(m.elo)})`).join(', ')}. Rankings are based on blind user votes in the Text to Video Arena.`,
              },
              {
                q: 'How are Text to Video models ranked on this leaderboard?',
                a: 'Models are ranked using an Elo rating system derived from user votes in blind comparisons in the Text to Video Arena. Users compare two videos generated from the same prompt without knowing which model created each video. Higher Elo scores indicate a model is preferred more often by users.',
              },
            ].map((faq, i) => (
